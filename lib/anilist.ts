...
const SEARCH_MEDIA_QUERY = `
  query SearchMedia($query: String!, $perPage: Int!, $countryOfOrigin: CountryCode) {
    Page(perPage: $perPage) {
      media(search: $query, type: MANGA, countryOfOrigin: $countryOfOrigin, sort: POPULARITY_DESC) {
        id
        title {
          romaji
          english
...
export async function searchMedia(query: string, perPage: number): Promise<AniListMedia[]> {
  const trimmed = query.trim();
  // An empty query isn't an error — it's just "nothing to search for yet" (e.g. the
  // user cleared the input) — no point round-tripping to AniList for it.
  if (trimmed.length === 0) {
    return [];
  }
  if (trimmed.length > MAX_SEARCH_QUERY_LENGTH) {
    throw new Error(`query tối đa ${MAX_SEARCH_QUERY_LENGTH} ký tự`);
  }
  if (!Number.isInteger(perPage) || perPage < MIN_PER_PAGE || perPage > MAX_PER_PAGE) {
    throw new Error(`perPage phải là số nguyên trong khoảng ${MIN_PER_PAGE}-${MAX_PER_PAGE}`);
  }
  ...
  try {
    response = await fetch(ANILIST_API_URL, {
      ...
      body: JSON.stringify({
        ...
        // countryOfOrigin is always "JP", never taken from the caller — see
        // JAPAN_COUNTRY_CODE above, same as fetchTrendingMedia/fetchMediaById.
        variables: { query: trimmed, perPage, countryOfOrigin: JAPAN_COUNTRY_CODE },
      }),
      signal: controller.signal,
    });
  } catch (error) {
